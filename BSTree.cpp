...
template<class Key> BSTree* BSTree<Key>::most_left(BSTree* start)
 {
  if(start->l_son != NULL)
...
template<class Key> BSTree* BSTree<Key>::lookForVal(BSTree* start, Key value)
{
...
template<class Key> int BSTree<Key>::addNode(Key n, BSTree* start)
 {
  //jezeli drzewo jest puste to dodaj korzen
  if (root == NULL)
   {
	root = (BSTree*)malloc(sizeof *root);
    root->value = n;
    root->l_son = NULL;
    root->r_son = NULL;
    root->parent = NULL;
  ...
  //jezeli zadana wartosc jest mniejsza od korzenia idz do lewego poddrzewa
  else if(n < start->value)
   {
    ...
    //jezeli lewe poddrzewo nie istnieje dodaj nowy wezel o zadanej wartosci
     ...
     {
	  BSTree* newNode = (BSTree*)malloc(sizeof *root);
      newNode->value = n;
      newNode->l_son = NULL;
      newNode->r_son = NULL;
...
//usun wezel start
template<class Key> void BSTree<Key>::delNode(BSTree* start)
 {
...
template<class Key> void BSTree<Key>::in_order_tree_walk(BSTree* start)
 {
